...
public:
	...
	int found=0;
	char payload[124]="{\"DD\":1,\"AD\":1,\"DW\":0,\"AW\":0,\"TD\":0,\"HD\":0,\"SD\":1,\"Dist\":00.00}";
    void notify() {
		...
		//memcpy(its_data,buff,strlen(buff));
			...
			its_size = serial_1(buff); //this is the data received from UWB and used to transmit on SOME/IP
			
			//the below payload format for UWB to SOA for various operations
			//{ "DD": 1 , "AD": 1 , "DW": 0 , "AW" : 0 , "TD" : 0 , "HD": 0 , SD : 1 , “Dist”: 2 }
			/*
			LB – load balancer.
			CU -Centralised ECU
			UWBECU – UWB transceiver ECU
			DD – driver door ECU     
			AD – all door ECU
			DW – driver window ECU
			AW – All window ECU
			TD – Trunk door ECU
			HD – Hood door ECU
			SD – sunroof door ECU
			*/
			...
			std::string str(payload);
    			found = str.find("\"Dist\":");
    			memcpy(payload+found+strlen("\"Dist\":"),buff,strlen(buff));
			memcpy(payload+strlen(payload)-1,"}",1);
			memcpy(its_data,payload,strlen(payload));
			//memcpy(its_data+strlen(payload),"}",1);
			

			//memcpy(its_data,buff,its_size);//strlen(buff));
                    std::lock_guard<std::mutex> its_lock(payload_mutex_);
